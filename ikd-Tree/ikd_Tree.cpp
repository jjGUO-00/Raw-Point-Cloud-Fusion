...
// ------ 2. 获取树的信息 ------
// 2.1 返回 ikd-Tree 上的节点总数，包括有效节点和无效节点。
template <typename PointType>
...
// 2.2 返回能包含所有点的bounding box，即包含两个数组的结构体，分别为每个坐标轴上的最小值和最大值。
template <typename PointType>
...
// 2.4 获取整个树的balance criterion value和delete criterion value
template <typename PointType>
...
'''
在 ikd树上搜索目标点 的 k 个最近邻点。 关键入口函数
输入：    
    point：目标点
    k_nearest：要搜索点的个数
    Nearest_Points：待返回的k个最近点
    Point_Distance：最近点到目标点的距离
    max_dist：搜索的范围限制（米）
'''
template <typename PointType>
...
输入：    
    root：指定的根节点
    Storage：返回的有效点集
    storage_type：enum，用于确定删除点在删除之前是否应存储在缓冲区中。
'''
template <typename PointType>
